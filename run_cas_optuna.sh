...
do
    ...
    python -m core.runCaS_optuna dataset $dataset gnn.model.name MLP seed $seed runs $runs cas.optuna.n_jobs 4 >> "${dataset_results}/${dataset}_mlp.txt" 2>> "${dataset_results}/${dataset}_mlp.err" &
    python -m core.runCaS_optuna dataset $dataset gnn.model.name GCN seed $seed runs $runs cas.optuna.n_jobs 4 >> "${dataset_results}/${dataset}_gcn.txt" 2>> "${dataset_results}/${dataset}_gcn.err" &
